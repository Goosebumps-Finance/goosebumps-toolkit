# Goosebumps Profile SDK

This package provides some handy functions to retrieve data for Goosebumps Profile system.
...
Install `@goosebumps/profile-sdk` into your project with npm:

```bash
npm install @goosebumps/profile-sdk --save
```

or yarn:

```bash
yarn add @goosebumps/profile-sdk
```
...
```js
import GoosebumpsProfileSdk from "@goosebumps/profile-sdk";

const goosebumpsSdk = new GoosebumpsProfileSdk();
```
...
const goosebumpsSdk = new GoosebumpsProfileSdk(customProvider, 97);
```
...
const goosebumpsSdk = new GoosebumpsProfileSdk();
const username = goosebumpsSdk.getUsername("0x123456789");
...
const goosebumpsSdk = new GoosebumpsProfileSdk();
const team = goosebumpsSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for goosebumps.finance domain, maybe configurable in future versions)

```js
import GoosebumpsProfileSdk from "@goosebumps/profile-sdk";

const goosebumpsSdk = new GoosebumpsProfileSdk();
const profile = goosebumpsSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = goosebumpsSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@goosebumps/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [goosebumps-frontend](https://github.com/CryptoSnowPrince/goosebumps-frontend) repo. There are several improvements to be made in the future versions of this SDK:
